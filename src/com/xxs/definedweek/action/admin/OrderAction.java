...
import com.xxs.definedweek.entity.Deposit.DepositType;
import com.xxs.definedweek.entity.Order.OrderStatus;
import com.xxs.definedweek.entity.Order.ShippingStatus;
import com.xxs.definedweek.entity.OrderLog.OrderLogType;
import com.xxs.definedweek.entity.Payment.PaymentStatus;
import com.xxs.definedweek.entity.Payment.PaymentType;
import com.xxs.definedweek.entity.Refund.RefundType;
...
public class OrderAction extends BaseAdminAction {
	...
	public String edit() {
		...
		if (order.getPaymentStatus() != com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) {
	...
	public String update() {
		...
		if (persistent.getPaymentStatus() != com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) {
			addActionError("此订单付款状态无法编辑!");
			return ERROR;
		}
		...
		for (OrderItem orderItem : orderItemList) {
			...
			if (product.getStore() != null) {
				...
				if ((setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) || 
				...
				if (setting.getStoreFreezeTime() == StoreFreezeTime.order || (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) || 
					(setting.getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() != ShippingStatus.unshipped)) {
	...
	public String payment() {
		...
		if (order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.paid || order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.partRefund || order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.refunded) {
		...
		// 库存占用数处理
		Setting setting = getSetting();
		if (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) {
		...
		order.setOrderStatus(OrderStatus.processed);
		if (payment.getTotalAmount().compareTo(order.getTotalAmount().subtract(order.getPaidAmount())) < 0) {
			order.setPaymentStatus(com.xxs.definedweek.entity.Order.PaymentStatus.partPayment);
		} else {
			order.setPaymentStatus(com.xxs.definedweek.entity.Order.PaymentStatus.paid);
		}
		...
		for (DeliveryItem deliveryItem : deliveryItemList) {
			...
			for (OrderItem orderItem : orderItemSet) {
				if (product == orderItem.getProduct()) {
					...
					if (product.getStore() != null) {
						if ((setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) || (setting.getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() == ShippingStatus.unshipped)) {
	...
	public String refund() {
		...
		if (order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.unpaid || order.getPaymentStatus() == com.xxs.definedweek.entity.Order.PaymentStatus.refunded) {
		...
		order.setOrderStatus(OrderStatus.processed);
		if (refund.getTotalAmount().compareTo(order.getPaidAmount()) < 0) {
			order.setPaymentStatus(com.xxs.definedweek.entity.Order.PaymentStatus.partRefund);
		} else {
			order.setPaymentStatus(com.xxs.definedweek.entity.Order.PaymentStatus.refunded);
		}
	...
	public String invalid() {
		...
		if (order.getOrderStatus() == OrderStatus.completed) {
			...
			return ajax(Status.error, "此订单已经作废!");
		} else if (order.getPaymentStatus() != com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) {
			...
			Setting setting = getSetting();
			if (setting.getStoreFreezeTime() == StoreFreezeTime.order || (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != com.xxs.definedweek.entity.Order.PaymentStatus.unpaid) || order.getShippingStatus() != ShippingStatus.unshipped) {
				for (OrderItem orderItem : order.getOrderItemSet()) {
					Product product = orderItem.getProduct();
					product.setFreezeStore(product.getFreezeStore() - orderItem.getProductQuantity());
					productService.update(product);
